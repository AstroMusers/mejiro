"""
Generates synthetic images: idealized images with no noise or detector effects
(optionally, convolved with PSF).

The YAML config's ``jaxtronomy.use_jax`` selects the execution path:

* ``use_jax: False`` -- lenstronomy ray-shooting in a fork-context
  ``ProcessPoolExecutor``.

* ``use_jax: True`` -- ray-shooting routed through jaxtronomy by flipping
  ``StrongLens.use_jax`` to True on every profile in ``_JAXXED_MODELS`` before
  constructing the SyntheticImage. Execution topology is selected from
  ...
  * ``gpu`` -- single-process sequential execution, bucketed by lens model
    signature so the JIT cache is reused within each bucket. One in-flight
...
Worker threading (JAX CPU path):
    ...
    2. ``XLA_FLAGS`` set in ``_jax_worker_init`` (unconditional assignment, not
...
# Populated per-worker by ``_jax_worker_init`` so create_synthetic_image can
# flip use_jax without re-importing jaxtronomy for every lens. Stays None on
# the non-JAX path, which is how the worker knows not to touch lens.use_jax.
...
def main(args):
    ...
    pipeline = PipelineHelper(args, args.prev_step, SCRIPT_NAME, SUPPORTED_INSTRUMENTS,
    ...
    if use_jax:
        ...
        # Co-locate the persistent JIT cache with the outputs so it survives
        # across runs and is shared between all spawn workers.
    ...
    if not use_jax:
        _run_no_jax(pipeline, tuple_list)
    elif jax_platform == 'gpu':
        _run_jax_gpu(tuple_list, compilation_cache_dir)
    else:
        _run_jax_cpu(pipeline, tuple_list, compilation_cache_dir)
...
def _verify_jax_platform(jax_platform):
    ...
    Importing jax in *this* process would lock its platform for the rest of the
...
def _run_jax_cpu(pipeline, tuple_list, compilation_cache_dir):
    """JAX CPU path: spawn-context process pool, one core-pinned worker per CPU id."""
    workers = pipeline.calculate_process_count(len(tuple_list))
...
def _run_jax_gpu(tuple_list, compilation_cache_dir):
    """JAX GPU path: bucket by lens_model_list signature, run in main process.
    ...
    """
    _jax_worker_init('gpu', compilation_cache_dir, None)
...
def _jax_worker_init(jax_platform, compilation_cache_dir, cpu_queue):
    """JAX worker setup: pin platform, enable JIT cache, cache JAXXED set.

    Serves both JAX paths: spawn-worker initializer on CPU (``cpu_queue`` holds
    the core ids to pin to), called directly in the main process on GPU
    (``cpu_queue`` is None, no pinning).
    """
...
def _enable_jax_on_lens(lens):
    ...
    lens.use_jax = [m in _JAXXED_MODELS_SET for m in lens.lens_model_list]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prev-step', dest='prev_step', choices=['02', '03'], default='03',
                        help='Pipeline step to read lens pickles from.')
