...
Worker threading (CPU path):
    ...
    2. ``XLA_FLAGS`` set in ``_worker_init`` when JAX is enabled
       (unconditional assignment, not setdefault, so an inherited shell value
       cannot mask it): ``--xla_cpu_multi_thread_eigen=false`` disables Eigen
       op-level parallelism; ``--xla_force_host_platform_device_count=1``
       keeps JAX to one logical CPU device per worker.
...
def main(args):
    ...
    if use_jax:
        # Verify JAX is importable and has devices for the requested platform.
        # Run in a subprocess so the main process's JAX platform stays
        # unlocked; we never import jax in this module.
    ...
    if use_jax:
        ...
        logger.info(f'Processing {len(filtered_tuple_list)} run(s) without JAX')

    if use_jax and jax_platform == 'gpu':
        num_detectable += _run_sequential(filtered_tuple_list, compilation_cache_dir, jax_platform)
    else:
        num_detectable += _run_spawn_pool(pipeline, filtered_tuple_list, compilation_cache_dir, jax_platform, use_jax)
...
def _run_spawn_pool(pipeline, tuple_list, compilation_cache_dir, jax_platform, use_jax):
    ...
    try:
        with ProcessPoolExecutor(
            ...
            initargs=(jax_platform, compilation_cache_dir, cpu_queue, use_jax),
        ) as executor:
...
def _run_sequential(tuple_list, compilation_cache_dir, jax_platform):
    ...
    """
    _worker_init(jax_platform, compilation_cache_dir, None, use_jax=True)
...
def _worker_init(jax_platform, compilation_cache_dir, cpu_queue, use_jax):
    """Spawn worker entry point: pin CPU affinity; when JAX is enabled, also
    pin the JAX platform, enable the JIT cache, and cache the JAXXED set."""
    ...
    # prevent greedy GPU memory preallocation; no-op on the CPU path
...
def run_slsim(tuple):
    ...
    speed_factor = survey_config['speed_factor']
    ...
    _snr_pos = snr_config.get('snr_detector_position', (2554, 2554))  # default to center of detector
