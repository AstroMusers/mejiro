...
@hydra.main(version_base=None, config_path='../../config', config_name='hwo_config.yaml')
def main(config):
    start = time.time()
    ...
    output_dir = os.path.join(config.machine.data_dir, 'hwo')
    ...
    if debugging: print('Configured HWO filter(s)')
...
def run_slsim(tuple):
    ...
    for candidate in tqdm(total_lens_population, disable=not debugging):
        snr, _ = survey_sim.get_snr_hwo(candidate, 'hdi')
        snr_list.append(snr)
