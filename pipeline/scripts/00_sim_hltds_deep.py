...
@hydra.main(version_base=None, config_path='../../config', config_name=f'config_{SURVEY}.yaml')
def main(config):
    start = time.time()
    ...
    if debugging:
        output_dir = os.path.join(f'{config.machine.pipeline_dir}_{SURVEY}',
...
def run_slsim(tuple):
    ...
    cache_dir = os.path.join(module_path, 'data', f'{SURVEY}_skypy_configs')  # hltds_wide_skypy_configs
    skypy_config = os.path.join(cache_dir,
                                f'roman_{SURVEY}_sca{sca_id}.yml')  # TODO TEMP: there should be one source of truth for this, and if necessary, some code should update the cache behind the scenes
