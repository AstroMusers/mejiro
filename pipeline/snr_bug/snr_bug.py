...
def main():
    ...
    pipeline_dir = os.path.join(data_dir, 'snr_bug')
    ...
    for run in range(runs):
        ...
        tuple_list.append((str(run).zfill(4), sca_id, config, output_dir, None, psf_cache_dir, roman))
...
def run_slsim(tuple):
    ...
    if survey_config['total_population']:
        ...
        snr_list = []
        num_exceptions = 0
        for i, candidate in enumerate(total_lens_population):
            ...
            util.pickle(os.path.join(output_dir, f'run{str(run).zfill(2)}_{str(i).zfill(8)}.pkl'), exposure)          
