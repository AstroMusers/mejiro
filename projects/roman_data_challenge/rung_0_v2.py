"""
Exports romanisim exposures and synthetic images to HDF5 format (rung 0).
...
Usage:
    python3 rung_0_v2.py --config <config.yaml>
...
def main(args):
    ...
    # load pre-computed SNR values
    snr_pkl_path = os.path.join(pipeline.pipeline_dir, 'snr', 'name_snr_pairs.pkl')
    if not os.path.exists(snr_pkl_path):
        logger.warning(f'SNR file not found: {snr_pkl_path}')
        snr_lookup = {}
    else:
        _raw_snr_pairs = util.unpickle(snr_pkl_path)
    ...
    for uid in tqdm(uids):
        ...
        for i, band in enumerate(bands):
            ...
            system_name = f'{pipeline.name}_{str(uid).zfill(8)}'
            snr_key = f'{system_name}_{band}'
            if snr_key not in snr_lookup:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Export romanisim rung 0 dataset to HDF5 format.")
