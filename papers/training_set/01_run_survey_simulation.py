...
def run_slsim(tuple):
    ...
    cache_dir = os.path.join(module_path, 'data', config['survey']['skypy_config_dir'])
    skypy_config = os.path.join(cache_dir,
                                f'{config["survey"]["skypy_config_prefix"]}_sca{sca_id}.yml')  # TODO TEMP: there should be one source of truth for this, and if necessary, some code should update the cache behind the scenes
